...
- **Zoho rate-limits the token-refresh endpoint itself**, separately from normal API call limits. Constructing a fresh `ZohoClient()` per request (e.g. per FastAPI request) refreshes the token every time and trips this quickly ("You have made too many requests continuously"). Use `zoho_client.get_shared_client()` — a process-lifetime singleton — anywhere a client is needed outside a single script run; `run_zoho()` / `run_zoho_for_job()` both accept an optional `client=` param so callers can pass the shared one through.
