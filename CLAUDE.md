...
- **`get_applications_for_job()` excludes `Application_Status == "Rejected"`** client-side — previously-rejected candidates shouldn't be re-surfaced and re-analyzed every time a job's applicants are pulled. Confirmed live on one job's 4,899 applications: `Application_Status` and the coarser `Hiring_Pipeline` field agreed exactly on which 258 were rejected. Note `"Junk candidate"` is a separate `Application_Status` value and is **not** excluded by this filter (only asked to exclude rejections, not junk).
