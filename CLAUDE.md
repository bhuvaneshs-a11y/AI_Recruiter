...
#### Resolved: "Best Match" going to the wrong candidate on tied fit scores

- **Error**: two candidates both scored `overall_fit_score=55`, but the one with `overall_credibility_score=50` was ranked "Best Match" over the one with `overall_credibility_score=95`.
- **Root cause**: `main._rank_key()` returned only the fit score as the sort key. When two results tie on that single number, Python's `sort()` is stable — ties keep whatever order they were already in *before* sorting, which is just whichever candidate's analysis happened to finish processing first (an incidental timing detail from concurrent processing, not a quality signal).
- **Fix**: `_rank_key()` now returns `(fit, credibility)` (or `(credibility, credibility)` when there's no job-fit context) instead of a bare number, so `results.sort(key=_rank_key, reverse=True)` breaks fit ties by credibility instead of leaving them to completion-order luck. Verified live with the exact reported scenario (fit 55/55, credibility 50/95): the 95-credibility candidate now correctly sorts first.
