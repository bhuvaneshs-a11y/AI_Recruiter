...
`_run_concurrent_until_target()` (vs. the unconditional `_run_concurrent()`) is what makes "keep searching until N matches" work — it submits candidates in chunks instead of all at once, stopping once enough successes exist. A chunk already in flight when the target is hit is allowed to finish rather than being cancelled, so results can slightly overshoot `limit` (confirmed live: asked for 3, chunk size 4 in flight, got 4) — a deliberate simplification, not a bug.
